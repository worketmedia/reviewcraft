...
const HIGHLIGHTS: Record<string, string[]> = {
  ...
  Cleanliness: ["Spotlessly clean", "Well-maintained washrooms", "Hygienic food preparation"],
  Value: ["Worth every rupee", "Great value for money", "Affordable prices", "Best deal in town"]
}
...
export default function ReviewFlow() {
      ...
      <div className="flex flex-col h-full py-8 flex-1 mt-8">
        ...
        <div className="mt-auto pt-10">
          <button
            ...
            className={`w-full py-4 rounded-xl font-semibold text-lg transition-colors min-h-[44px] ${overallRating > 0
  ...
  const renderStep4 = () => {
    ...
    const highRatedSections = Object.entries(categoryRatings)
      .filter(([cat, rating]) => rating >= 4)
      .map(([cat]) => categoryMapping[cat])
      .filter(Boolean)

    let sectionsToShow = Object.entries(HIGHLIGHTS)
    if (highRatedSections.length > 0) {
      sectionsToShow = sectionsToShow.filter(([section]) => highRatedSections.includes(section))
    }
                ...
                {tags.map(tag => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 py-2 min-h-[44px] rounded-full text-sm border transition-colors ${isSelected
      ...
      <div className="flex flex-col items-center h-full py-12 flex-1 mt-8 text-center">
        ...
        <div className="mt-auto w-full space-y-4">
          ...
          <button
            ...
            className={`text-sm font-medium py-2 min-h-[44px] w-full text-center transition-colors ${copied ? 'text-green-600' : 'text-gray-500 underline'
              }`}
          >
